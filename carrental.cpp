...
void SetData(string n, string ad, string id, string num) {
name = n;
address = ad;
AdharId = id;
Number = num;
...
void Person_Info(Person & p) {
bool check = false;
string fname, lname, name, Number, Aid, address;
int a = 0;
...
do {
a = 0;
cout << "Enter your AdharID ( 12 Digits without Dashes... ) : ";
getline(cin, Aid);
...
} while (!(Aid.size() == 12) || check == true); // ADHAARID
...
p.SetData(name, address, Aid, Number); // Setting values to person class setdata function
}
